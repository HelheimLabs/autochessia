...
library EventLib {
    ...
    function _pack(Event memory _eve) pure internal returns (uint256 output) {
    ...
    function parseEvent(uint256 _input) pure internal returns (Event memory eve) {
        eve = Event({
    ...
    function genOnAttack(uint256 _attackerIndex, uint256 _targetIndex, uint256 _power) internal pure returns (uint256 eve) {
        eve = Event({
    ...
    function genOnCastSpell(uint256 _casterIndex, uint256 _targetData, uint256 _spell) internal pure returns (uint256 eve) {
        eve = Event({
    ...
    function genOnDamage(uint256 _receiverIndex, uint256 _source, uint256 _damage) internal pure returns (uint256 eve) {
        eve = Event({
