...
library EffectLib {
  ...
  function getEffectIndex(uint24 _effect) pure internal returns (uint16 index) {
    return uint16(_effect >> 8);
  }

  function getEffectDuration(uint24 _effect) pure internal returns (uint8 duration) {
    return uint8(_effect);
  }

  function getEffectEventType(uint24 _effect) pure internal returns (uint8 eventType) {
    return uint8((getEffectIndex(_effect) & EFFECT_EVENT_TYPE_MASK) >> 12);
  }

  function effectHasModification(uint24 _effect) pure internal returns (bool has) {
    return getEffectIndex(_effect) > EFFECT_WITH_MODIFIER_MASK;
  }

  function effectHasTrigger(uint24 _effect) pure internal returns (bool has) {
    return getEffectEventType(_effect) > 0;
  }

  function effectTriggerOnDirect(uint24 _effect) pure internal returns (bool isDirect) {
  ...
  function _applyModification(RTPiece memory _piece, uint24 _modification, uint256 _multiplier) pure private {
  ...
  function _applyChangeInfo(uint256 _original, uint16 _changeInfo, uint256 _multiplier) pure private returns (uint256 updated) {
  ...
  function effectMatchEvenType(uint24 _effect, EventType _eventType, bool _isDirect) pure internal returns (bool matched) {
    if (
      getEffectEventType(_effect) == uint8(_eventType) &&
      effectTriggerOnDirect(_effect) == _isDirect
    ) {
      matched = true;
    }
  }
  ...
  function parseTrigger(EffectCache memory _cache, uint24 _effect) view internal returns (
    bool[EFFECT_NUM_IN_TRIGGER] memory xs,
    uint256[EFFECT_NUM_IN_TRIGGER] memory applyTos,
    uint256[EFFECT_NUM_IN_TRIGGER] memory effects,
    uint256 subAction
