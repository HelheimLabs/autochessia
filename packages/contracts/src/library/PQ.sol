...
library PQ {
    ...
    function IsEmpty(PriorityQueue memory _pq) internal pure returns (bool) {
        return _pq.num == 0;
    }

    function AddTask(PriorityQueue memory _pq, uint256 _data, uint256 _priority) internal pure {
    ...
    function PopTask(PriorityQueue memory _pq) internal pure returns (uint256 data) {
        if (_pq.num > 0) {
            data = _pq.data[--_pq.num];
        }
    }
