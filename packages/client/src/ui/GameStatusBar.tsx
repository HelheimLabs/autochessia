  ...
  return (
    <div className={`flex justify-center  mx-auto mt-1`}>
      ...
      <div className="w-[300px] relative text-center">
        <span className="w-[80px] relative text-center  inline-block"> Time left:</span>
        <span className="w-[80px] relative text-center  inline-block">
          {startBlockNumber >= 0 ? dayjs.duration(startBlockNumber, 'seconds').format('mm:ss') : '--:--'}
        </span>
        <div className={`${width <= 0 ? 'bg-transparent' : 'bg-blue-500'} transition-all absolute inset-x-0 top-0 mx-auto h-[25px] -z-10 rounded-lg`} style={{ width: width + '%' }} >  </div>
